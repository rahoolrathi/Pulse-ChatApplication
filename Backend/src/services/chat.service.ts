...
import { GroupMemberRole, RoomType, MessageType } from "../utils/helper.util";
...
export class ChatService {
      ...
      const messageResponse = {
        ...
        group: group
          ? {
              ...
              name: group.name,
              description:
                group.description === null ? undefined : group.description,
            }
          : undefined,
      };
  ...
  static async cacheMessage(message: MessageResponse): Promise<void> {
    try {
      ...
      if (message.recipientId) {
        conversationKey = this.getPrivateConversationKey(
          message.senderId,
          message.recipientId
        );
  ...
  static async createGroup(groupData: CreateGroupData) {
    try {
      ...
      if (groupData.memberIds && groupData.memberIds.length > 0) {
        const filteredMemberIds = groupData.memberIds.filter(
          (id) => id !== String(groupData.createdBy)
        );
        ...
        const missingUserIds = filteredMemberIds.filter(
          (id) => !existingUserIds.includes(id)
        );

        if (missingUserIds.length > 0) {
          throw new Error(
            `Users not found with IDs: ${missingUserIds.join(", ")}`
          );
        }
  ...
  static async createDirectChat(userId: string, otherUserId: string) {
    try {
      ...
      const data = {
        ...
        avatar_url: otherUser.profile_picture || undefined,
        type: "direct",
      };
      console.log(data);
      return data;
